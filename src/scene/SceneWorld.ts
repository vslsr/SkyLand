...
export class SceneWorld implements GrassInteractionTarget {
  ...
  public setWeaponDraw(actorId: string | undefined, charge: number): void {
    this.actorSnapshotTarget?.setWeaponDraw?.(actorId, charge);
  }

  /** 这把弓松了：拉弓量归零。回弹是另一件事，见 `releaseHeldWeapon`。 */
  public clearWeaponDraw(actorId: string | undefined): void {
    this.actorSnapshotTarget?.clearWeaponDraw?.(actorId);
  }

  /** 撒手那一下：弦回弹抖两下。一次性事件。 */
  public releaseHeldWeapon(actorId: string): void {
    this.actorSnapshotTarget?.releaseHeldWeapon?.(actorId);
  }
