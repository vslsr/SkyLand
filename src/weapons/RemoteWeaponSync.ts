...
/**
 * 别人手上那把武器（设计稿 `@i 木弓` / `@i 弹弓` 的 `A`，多端那一半）。
 *
 * 拉开和撒手都是**公开的**：屋里每个人都该看见别人的弓弯没弯、皮筋拉没拉开、
 * 什么时候松的手。弓和弹弓在这一层没有区别——过来的是同一个「拉了几成」。
...
export class RemoteWeaponSync {
   ...
   *
   * 两端跑同一个公式，所以旁观者看到的拉开程度和射手自己看到的是同一个；下发算好
  ...
  private applyCharge(
    ...
    if (!charge || !heldActorId) {
      this.port.clearWeaponDraw(heldActorId);
      return;
    }
    this.port.setWeaponDraw(heldActorId, holdRatio(
   ...
   */
  private applyShot(entry: WeaponBearer, heldActorId: string | undefined): void {
    ...
    if (heldActorId) this.port.releaseWeapon(heldActorId);
  }
}
