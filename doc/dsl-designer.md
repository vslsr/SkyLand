...
`@`+**单个字母**是一条定义（`@i` `@b` `@w`），`@`+**单词**是一个标记（下面三个）
`<>`符号内为此段落对应的含义
...
* 全部记法都以 `@` 开头，靠**后面是字母还是单词**区分两类：
    * `@` + **单个字母** = 一条**定义**：`@i` `@b` `@w` `@e`。它起一个条目，后面跟名称和字段
    * `@` + **单词** = 一个**标记**：`@todo` `@design` `@advice`。它不起条目，只是标住某处还差什么
  以后加类型就取一个没用过的字母，加标记就取一个单词，这条规则不会撞车。
...
**位置不决定用哪个标记，起点才决定。** 三个都能独占一行、也能标在某一行的行尾，
`@design` 还能直接填在值的位置。所以下面几种都是对的：
...
三个标记统一用 `@` 而不是 `#`，是因为 **`#` 在 Markdown 行首就是标题**：`#design` 单独
占一行会渲染成一个 H1「design」，而标记恰恰要能独占一行（`@todo` 就常写在模块标题
上一行）。`@` 不是 Markdown 的语法字符，三个标记因此可以放在任何位置。也别改用
`<!-- -->` 包起来——注释在渲染出来的文档里**看不见**，而「标了就看得见」正是这些
标记唯一的作用。
