...
* 不带参数的 `#design` 按 `#design(do)` 办：**只设计、不写代码**
...
* 状态取值：待机、手持、蓄力、使用、命中、建造、拆除、交互、工作、冷却
* 驱动量四选一，写在状态说明里：
...
* `#design` 放在值的位置，意思是**这一条故意留白，由实现方提出一个合适的设计**：
  ...
  （`(do)` / `(w)` / `(s,n)`），不带参数只设计不写代码；连同它和 `@todo` 的分工，
  见下面「标记」一节。
...
两个都表示「这里还差点什么」，但差的**不是同一样东西**，读到时要做的事也不同：
...
所以 `#design` 出现在**值的位置**（`T: #design`），`@todo` 标的是**整块或整行**。
一处标了 `#design` 就隐含还没实现，不用再补一个 `@todo`；反过来不成立——
...
`#design` 不只是「这里缺个设计」，它还说明**设计完之后该做什么**。参数写在括号里：

| 写法 | 叫什么 | 设计完做什么 |
| --- | --- | --- |
| `#design(do)` | design-only | 把设计填回 md 的这个位置，去掉 `#design`，这条改标 `@todo`。**不写代码** |
| `#design(w)` | write | 把设计填回 md，**并且直接实现**。实现完不留 `@todo` |
| `#design(s,n)` | select | 给出 **n 个不同方向**，说清各自的取舍，**问用户选哪一个**。不要自己替用户定 |
| `#design` | 不带参数 | 按 `do` 办 |

**不带参数按 `do`**：只设计不写代码。这条是故意保守的——把一处只想要方案的留白
...
三个标记状态连起来就是一条内容的完整生命周期：

```text
#design(...)          缺设计
      │ 设计完，填回 md
      ▼
@todo                 有设计，缺实现        ← #design(do) 停在这里
      │ 实现完，去掉标记
      ▼
（无标记）             已落地               ← #design(w) 一步走到这里
```

所以 `#design` 处理完**一定会留下痕迹**：要么变成 `@todo`，要么变成没有标记的成品。
留着原样的 `#design` 意味着这一处还没被处理过。
