...
function createMushroom(): {
  ...
  // 渲染根位置平时由 ActorTransformSystem 写 SoA、渲染侧兑现；这里只测翻滚，直接摆好即可。
  render.root.position.set(1, 0, 2);
  world.addActor(actor);
  return { world, actor, scene, render };
}
...
test('还长在地上时翻滚系统不改姿态', () => {
  const { world, scene, render } = createMushroom();
  const system = new ActorDropRollSystem(scene);
...
test('脱落后按刚体朝向翻倒，且绕球心而不是绕菌柄根部', () => {
  const { world, actor, scene, render } = createMushroom();
  const system = new ActorDropRollSystem(scene);
...
test('脱落之后弹性拉伸表现交出姿态，不再把蘑菇掰回竖直', () => {
  const { world, actor, scene, render } = createMushroom();
  const tetherVisual = new ElasticTetherVisualSystem(scene);
  const rollSystem = new ActorDropRollSystem(scene);
  const rig = render.elasticTetherRig;
  assert.ok(rig);
