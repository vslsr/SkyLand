...
**`纯表现` 是默认，`需复制` 走动作状态那条通道。** 角色动作现在有自己的通道了
（[`doc/animation-state-machine-design.md`](../../../../doc/animation-state-machine-design.md)）：
过网的是**状态**——`<动词>.<相位>` + 权威开始时刻 + 时长——曲线仍然是渲染侧的代码。
吃东西、拉弓走的都是它，别人看得见。

所以 `A` 里的一个动作要让别人看见时，落地的形状是固定的两步：服务端在动作开始 /
结束那两个时刻往 `ActionStateComponent` 写一次，客户端往
[`src/animation/ActionClipRegistry.ts`](../../../../src/animation/ActionClipRegistry.ts)
注册一条 `(状态, 角色) → 曲线`。**别再为一个动作单开一条网络状态。**

两类东西不走它：**世界物件**（树、石头）今天是按 chunk 烘进网格的，它们的动作要先
有一条按物件的实例通道；**连续的开合**（箱盖）过一个目标值 + 渲染侧回弹更合适——
状态机说的是「在做什么」，不是「现在开到几度」。
