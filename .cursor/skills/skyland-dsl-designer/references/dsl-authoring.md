...
- `#design` in a value position is a deliberate blank for the implementer to answer. An absent line is an omission, not a blank. Its parameter says how far to go: `(do)` design and mark `@todo`, `(w)` design and implement, `(s,n)` offer n directions and ask. Bare `#design` means `(do)` — never write code for it.
