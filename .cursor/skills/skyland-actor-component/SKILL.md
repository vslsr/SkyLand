...
3. Trace the affected path before editing:
   ...
   - Replica and presentation: `src/network/protocol.ts`, `src/scenes/data/SceneDefinition.ts`, `src/actors/ClientActorSystem.ts`, and the relevant model/visual System.
...
- Put Shader waves, bobbing, deformation, selection outlines, and smoothing in client visual Systems. Apply them to `visualRoot`; keep the replicated Actor root on the authoritative Transform.
...
`actor JSON -> Actor Schema -> ActorCatalog -> shared Component -> ServerActorFactory -> server System/interaction -> snapshot -> client protocol/types -> ClientActorSystem -> model/visual System -> HUD or feedback`
...
- A client child Actor root may be parented only under the parent Actor's authoritative root, never its `visualRoot`. Inherited bob/tilt is composed under the child's presentation-only attachment root.
...
3. Client tests cover heterogeneous Replica creation, snapshot application/removal, discrete reparenting, world-Transform interpolation, picking, feedback state, and `visualRoot`-only motion.
