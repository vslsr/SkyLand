...
**The anchor has to follow the grip around the shell.** Fixing the contact at grab time is right only while the teeth stay over that patch. Once the source has moved to another face — circled the victim, or crossed over them — `grip − patch` points almost entirely *inward*, the guard above strips that half, and all that survives is the outward floor along the **old** normal: the spike points away from the source. So when the direction from the body centre to the grip falls more than `REANCHOR_ALIGNMENT` off the stored normal, re-resolve the contact where the teeth now are and bump `revision` — the patch slides around the body to meet them. It must be a *new grab*, not an edited coordinate: the receiver bakes its influence weights around one vertex and only rebuilds them when the revision changes. The threshold gives its own hysteresis, since a fresh anchor starts perfectly aligned.
...
1. Server tests for a new source: grab refused when out of range/blocked, pull tracking both poses, **the pull pointing outward** (dot it with the contact normal — an inward pull is the classic direction bug), **the contact on the side the source is actually on** (a sign check on the axis between them: it is the only cheap sentinel for the shell-frame mistake), **the tip landing on the grip point** (contact + pull ≈ grip), auto-release at the break distance, cleanup when either side leaves. Sweep the source *through* and past the victim rather than testing one pose: the direction bugs all live at the poses where the grip crosses the shell, and a single face-off pose passes every one of them.
