...
**Every hold is its own vector, and the terms add.** N mouths mean N tips, each growing along its own axis, and the solver sums their displacements — that is why a hold must be a *field* term rather than a grabbed patch: patches would have to arbitrate over shared vertices, while displacement fields just add. Where two cones overlap the weights are normalised to sum to 1, so close-together mouths pull one broad ridge instead of stacking into a double-length spike, and the displacement at any point stays inside one tip's budget with no hard clamp anywhere.
...
- **One self-reported drag, up to `MAX_SOFT_BODY_HOLDERS` external holds.** The two halves are different in kind, so they count differently: the mouse drag is a single gesture on a picked point, while holds pile on — every extra mouth is another vector and another tip. Any external hold outranks the owner's own drag (`applySelfReported` refuses while one is held; on the render side `applyReplicated` is ignored while a local drag is active). The cap is a shared constant because the param SoA is fixed-length; past it, `grab` refuses.
...
- **Several ropes, one leash.** `stepCharacter` takes a single leash, and a slack rope exerts nothing, so the victim is restrained by the *tautest* hold (`tautestHold()`). Pulling toward it slackens it, another becomes tautest, and the victim settles where the holds jointly reach — instead of being yanked between two anchors by an averaged force that belongs to neither.
