...
- **Change what grows on a biome** (density, prop mix): edit the `BIOME_PROP_STYLE` table in both `shared/world/chunkContent.mjs` and `native/chunkgen/src/placement.rs`, rebuild the WASM, commit the binary. Keep every scale at or below 255 — the table only thins content out, which is what keeps `MAXIMUM_PROPS_PER_CHUNK` and the vertex budget unchanged. The grassland row is all 255 on purpose: it reproduces the pre-biome world bit for bit, so a change there is a change to every existing map.
