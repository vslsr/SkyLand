...
| `position` | `[x, y, z]` 三个数字 | `fly` 模式下是自由相机初始世界坐标；`topdown` 模式下是相对阻尼角色焦点的完整偏移。当前玩法配置统一为 `[5.5, 7.5, 8.5]`，形成从地面 XZ 斜方向俯视的构图。 |
...
加入房间时系统总会把这些字段写入备用自由相机；`topdown` 随后切换到 `TopDownController`，把完整 `position` 作为相对角色焦点的固定偏移，并只对焦点位置做阻尼追踪。`yaw`、`pitch`、`moveSpeed` 仍只对 `fly` 的实际画面和移动生效。所有字段继续必填，以保持统一配置契约。
