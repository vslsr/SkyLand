...
test('弓手转向玩家、拉满了才放，放出去的是同一条判定', async () => {
  ...
  // 蓄力比例由距离反解，所以八米外的那一发不会是拉满的十成。
  assert.ok(archer.requireComponent(WEAPON_SHOT_COMPONENT).ratio < 1);
...
test('弓手那一发也进快照：客户端不需要知道射手是不是玩家', async () => {
  ...
  // 和玩家那条是同一个形状：出手点 + 落点 + 蓄力比例。
  assert.deepEqual(Object.keys(shot).sort(), ['impactX', 'impactZ', 'ratio', 'revision', 'x', 'y', 'z']);
});
