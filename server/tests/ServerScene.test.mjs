...
test('史莱姆拖拽形变只被净化转发：换抓取时递增 revision，超时后自动清除', () => {
  const clock = createClock();
  const scene = new ServerScene('grassland', { now: clock.now });
  scene.addPlayer({ id: 'player-1', name: '旅人', slot: 0 });
  const readDrag = () => scene.createSnapshot().players[0].slimeDrag;
  const spawnX = scene.createSnapshot().players[0].x;

  assert.equal(readDrag(), undefined, '没有拖拽时不应下发字段');

  scene.applySlimeDrag('player-1', {
    contactX: 0, contactY: 0.9, contactZ: 0.1,
    pullX: 0.4, pullY: 0.2, pullZ: 0,
  });
  const first = readDrag();
  ...
  scene.applySlimeDrag('player-1', {
    contactX: 0, contactY: 0.9, contactZ: 0.1,
    pullX: 0.7, pullY: 0.2, pullZ: 0,
  });
  assert.equal(readDrag().revision, 1);
  assert.equal(readDrag().pullX, 0.7);

  // 松手后抓到另一处：接收端需要重建影响权重，所以 revision 递增。
  scene.applySlimeDrag('player-1', {
    contactX: 0.6, contactY: 0.4, contactZ: 0.1,
    pullX: 0.1, pullY: 0, pullZ: 0,
  });
  assert.equal(readDrag().revision, 2);

  // 越界与非法数值整体作废，不能把畸形负载转发给其他客户端。
  scene.applySlimeDrag('player-1', {
    contactX: 0, contactY: Number.NaN, contactZ: 0,
    pullX: 0, pullY: 0, pullZ: 0,
  });
  assert.equal(readDrag(), undefined);
  scene.applySlimeDrag('player-1', {
    contactX: 0, contactY: 1e9, contactZ: 0,
    pullX: 0, pullY: 0, pullZ: 0,
  });
  assert.equal(readDrag().contactY, 4, '超界坐标被夹到允许的最大本地偏移');

  // 客户端掉线或漏发松手时，拖拽不能永远留在快照里。
  clock.advance(2);
  assert.equal(readDrag(), undefined);

  scene.applySlimeDrag('missing-player', {
    contactX: 0, contactY: 0, contactZ: 0, pullX: 0, pullY: 0, pullZ: 0,
  });
  assert.equal(scene.createSnapshot().players.length, 1);
});
