...
草不走 chunk：`GrassFieldSystem` 在两种场景里都在跑，区别只是实例布局。
固定场景用 `FixedGrassLayout` 一次性铺满活动区；流式场景用 `RollingGrassLayout`，
一块跟着焦点滚动、80 米见方的实例网格，实例总数与世界大小无关。踩踏交互两边
都有，chunk 里因此不再摆草丛（模板注册为空），避免两套草叠在一起。
