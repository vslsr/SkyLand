...
手上有东西时交互键也是**按住**语义：短按放下、按住 `stowHoldSeconds` 收进背包。
它和物品能力的长按共用同一套表现：`HotbarController` 把按住时长翻译成
...
- 属于物品栏的那次（手持物品）画在那一格上——一圈真正的环盖在格子上，它已经同时
  说清了「哪一格」和「还要多久」；
- 没有格子的那些——叼着的蘑菇是个世界物件，从背包里点出来的用法根本不经过手——画在
  准星下方的 `HoldProgressBadge` 上。

两处一起亮不是冗余保险，是让玩家的眼睛在画面两端来回找同一件事。后者本身仍是必需
的：按键一落下，交互提示那条「E · 收进背包」就开始淡出（`InteractionPromptFade` 把
「正在操作」当成让开画面的信号），不补这一块，没有格子的那些按住之后画面上什么都
不剩。
...
（太远、已有东西、要放在水里、被人挡住、材料不够……）。按交互键把**格坐标**发给服务端，
...
水上地基放在开阔水面上时，服务端先立一个看不见的**船体根节点**（`float-hull`：浮力 + 载具马达 +
建造网格），板成为它的第 (0, 0) 格；后来的地基前后左右吸附在已有甲板旁边，墙吸附在甲板的格边上，
篝火放在甲板中心。每块板都进这艘船的浮力结算：板多了船更稳，墙和物件只加重量，装太多就会进水。
根节点带载具马达，走现有的载具接管，所以拼出来的船可以直接开。
