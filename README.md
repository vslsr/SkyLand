...
| `/admin` | 运维后台页面（独立文档，不加载游戏运行时） |
| `/api/admin/*` | 运维后台接口，需要口令登录 |
...
`/admin` 是一张独立页面：口令登录后可以看服务器总览、管在线玩家、管房间、查地图目录、
翻网关日志、改运行期设置。它与玩家侧完全隔离——玩家客户端没有任何入口，登录态也各走各的 cookie。

后台默认**关闭**，配了口令才打开（不配就等于把运维面板裸放在公网上）：

```powershell
$env:SKYLAND_ADMIN_PASSWORD = '至少八位的口令'
npm start
```

| 环境变量 | 作用 |
| --- | --- |
| `SKYLAND_ADMIN_PASSWORD` | **初始**口令。设置文件里一旦有了口令，改口令只能在后台做 |
| `SKYLAND_DATA_DIR` | 设置文件目录，默认 `<工作目录>/data`，文件名 `admin-settings.json` |

口令只以 scrypt 哈希落盘，服务端不留明文，接口也不回传。忘记口令的恢复办法：停服后删掉
设置文件里的 `passwordHash` 字段（或整个文件），重启即回到 `SKYLAND_ADMIN_PASSWORD`。

后台能改的运行期设置会立刻生效并写进设置文件：

| 设置 | 说明 |
| --- | --- |
| 最大房间数 | 0 = 不限制。达到上限后 `POST /api/rooms` 返回「服务器房间已满，请稍后再试」 |
| 空房回收（秒） | 房间空置多久后自动关闭，释放房间进程 |

「在线玩家」面板列的是当前全部 WebSocket 连接——**包含还停在大厅、没进房的那些**，
可以按连接断开某个人：服务端先发一条 `connection:kicked` 说明原因（客户端据此在大厅
显示提示，而不是「连接莫名断了」），再退房并以关闭码 4001 关掉 socket。

「服务器日志」面板读的是网关进程 console 的环形缓冲（默认 500 条，重启清空）。房间子进程的
输出直接继承到终端、不在缓冲里；房间异常退出由父进程记一条，后台能看到。
...
世界里的树和石头是种子的确定性产物，走到哪儿它们就在哪儿。生物不是：它们**什么时候
出、出在哪儿、什么时候消失**都由房间自己决定，而且要在一个 65 公里见方的世界上决定。

规则照 Minecraft 的自然刷新抄，抄的是规则的形状不是数值：
...
| 128 格外立刻消失、32–128 格随机消失 | `despawnVerdict` 的两档 |
| 成群刷新（一次 1–4 只） | `packMaximum` |
| 光照等级 ≤ 7 | 昼夜时钟的夜间窗口 `nightOnly` |
| `SpawnPlacements`（脚下实心、头顶有空间） | 平坦陆地 + 圆形足迹塞得下 |

规则全在 `shared/world/creatureSpawn.mjs`，是一层纯函数：不认识 Actor，不认识场景，
所以每一条都能单独测。什么时候问它们是 `server/scene/SceneCreatureSpawner.mjs` 的事。

### 出怪的地图也是种子的纯函数

Minecraft 的史莱姆区块只有约十六分之一的区块出史莱姆，而且这件事由种子决定、永不
改变。它把「随机刷怪」变成了一件玩家可以记住、可以规划的事——那一片沼泽一直出，
另一片一直不出。这里原样保留：`chunkOneIn` 写几就是几分之一的 chunk 出这种生物，
判定走的是和地形、物件同一个整数哈希。

成群刷新时**同伴各自再过一遍完整判据**，不是只沾队长的光。少了这一条，一个站在
刷新区块边上的刷新点会把半群撒进旁边那个不出怪的 chunk，「那片地一直出」这句话
就不再成立。

### 上限跟着人头走，不跟着面积走

这是与 Minecraft 唯一一处刻意的分歧。它的配额跟着已加载区块数走；SkyLand 的世界
太大，任何跟面积沾边的量都会立刻失控，而房间容量是写在场景里的硬上限。两种口径
想说的其实是同一句话——「一个人身边最多有这么多只」——按人头算更直接。

房间里没有人时配额是 **0**，不是「按最低配额留几只」：没有人看的房间里刷怪是纯粹的
浪费，而且它们会在下一个人进来之前把配额占满，让他进门就被围住。

尝试次数也**不**随缺口放大。配额空着说明这一带刷不出来（水多、树密、刷新区块少），
那时该让刷新变稀，而不是让这一个周期把 CPU 花在同一片刷不出的地上。

### 你铺过的地不刷怪

Minecraft 用光照做到「把屋子点亮，屋里就不出怪」。这里换成一条更直接的规则：
**落点所在的格上有建造件就不刷**。它给了建造一个防守上的理由，而不只是一个装饰。

「塞不塞得下」用的是玩家移动那份圆形推出，足迹半径取自这只生物**自己那份碰撞盒**的
推导函数——刷新、寻路、移动三处问的是同一个圆，所以不会刷出一只当场被推开的怪。

### 场景字段：`gameplay.creatureSpawns`

| 字段 | 作用 |
| --- | --- |
| `archetype` | 刷什么。原型自动进入场景原型表，不必在 `runtimeActorArchetypes` 里重复写 |
| `capPerPlayer` / `maximumPerRoom` | 每人一份配额，全房间再封一次顶 |
| `cycleSeconds` / `attemptsPerCycle` | 多久跑一次周期、一个周期试几个候选点 |
| `packMaximum` | 一次成群刷出来的上限 |
| `minimumDistance` / `maximumDistance` | 刷新圆环的内外径 |
| `chunkOneIn` | 几分之一的 chunk 出这种生物 |
| `nightOnly` | 只在夜里刷（需要场景开启 `environment.dayNight`） |

消失距离**不是**独立的配置项：出了 `maximumDistance` 随机消失，出了它的一倍半立刻
消失。一个和刷新范围脱钩的消失距离几乎总是配错的那一个。

`SceneCatalog` 在启动时校验：只能用在流式场景上（刷新要问「这一格是平地吗」，那个
问题只有台阶地形答得出来）、原型要有 `render`、要声明 `replicationPolicy.mode = aoi`
（刷出来的个体散在世界各处，全房间复制会让远处的客户端收到一堆它永远看不见的东西）、
外径必须大于内径（否则那条规则一只都刷不出来，而它看起来完全正常）。

### 绿史莱姆

`config/actors/green-legged-slime.actor.json` 是这套机制目前唯一的落点：一只绿色的
骨骼腿史莱姆，带 `navigation` 会自己找路追人（见「AI 寻路」），`replicationPolicy`
走 AOI。颜色写在原型的 `render` 上，不是刷新时挑的——换一种颜色就是换一个原型，
刷新那一层一个字都不用改。骨骼腿史莱姆那张图上开了它的刷新。

### 边界

- 刷出来的个体**不持久**：房间重启不保留，走远了就消失。Minecraft 用挂牌
  (persistence) 把一只怪钉住，那要的是一条玩家能操作的路径，现在还没有。
- 配额是全房间共享的，一个周期只按一个玩家取候选点。要「每个玩家各有一份」得先有
  分组。
- 只有一个刷新类别。Minecraft 分怪物／生物／环境／水生四类，各有各的配额；分类要
  等到真的有第二种生态位再做，现在做出来会是一个只有一行的表。
...
- `shared/world/creatureSpawn.mjs`：生物自然刷新的规则——刷新区块、配额、圆环采样、落点判据与消失判定
