...
手上有东西时交互键是**按住**语义：短按放下、按住 `stowHoldSeconds` 收进背包；
使用蓄力走同一条路。两者都由 `HotbarController` 把按住时长翻译成 `HeldItemProgress`，
判定仍在服务端——两端跑同一个 `chargeRatio`，所以圈满那一刻就是服务端判定满的那一刻。

进度画在两处，因为它们说的不是同一件事：

- 快捷栏当前那一格上的圈说「是手上这件东西」；
- 准星下方的 `HoldProgressBadge` 说「手该按着什么、还要多久」。

后者是必需的，不是重复：按键一落下，交互提示那条「E · 收进背包」就开始淡出
（`InteractionPromptFade` 把「正在操作」当成让开画面的信号），而叼着的蘑菇是个世界
物件、在快捷栏里没有格子，连圈都没有——不补这一块，玩家按住之后画面上什么都不剩。
...
- **使用** → `hold`，然后关掉背包。**它不代按一次使用键**：这个项目里的 `use` 是一个
  世界动作，投掷要蓄力（`mode: 'charge'`，菜单里按不出力度），工具要面前正好有可
  采集的目标（背包盖着画面时玩家根本没在瞄）。代按一次得到的只会是一次脚下的软投
  或一次打空的敲击。菜单能诚实做到的是把它送到手上并让开画面。
- **装备** → `assign` 到快捷栏。落点顺序和 `InventoryComponent.holdItemType` 一样：
  已经配着它的那格 → 第一个空格 → 当前选中格。已经在栏上时这一条列出来但点不动，
  菜单在每一格上长得一样，玩家不用重新数。
- **丢弃** → `drop:stack`，直接从账本扣一个、在身前生成掉落物。它和 `drop` 是两条：
  `drop` 丢的是手上那件，走「先 hold 再 drop」会顺手改写快捷栏一格、把原本握着的
  东西换下去，丢完还自动再抽一个同类的到手上——玩家只想扔掉一个石头，回头发现锤子
  进了包、手里攥着另一块石头。
