...
| 字段 | 作用 |
| --- | --- |
| `variants` | 这个种群长什么样的带权列表。原型自动进入场景原型表，不必在 `runtimeActorArchetypes` 里重复写 |
...
一条规则带一张**带权变体表**，长相是每一只落地那一刻掷出来的。同一份配额、同一批
刷新区块、同一套判据，出来的却是一群颜色各异的骨骼腿史莱姆。

写成三条规则也能凑出三种外观，但那样每一种各占一份配额，世界里就永远是「每种各两只」，
而不是「一共几只，样子各不相同」。所以变体在规则**里面**，不是并列的三条规则。

掷骰子的时机在判据**之前**：足迹是按变体存的（胖的那种半径更大），先判后掷会把一只
胖的塞进一个只够瘦的站的缝里。

这一层与世界物件的 `selectWorldPropVariant` 是两件事，别拿来复用：那一个必须由种子
确定性推导（两端各算一次要得到同一棵树），这一个是房间掷的骰子——刷出来的个体本来就
不可推导，它整只都要复制过去。

### 野生的骨骼腿史莱姆

目前三种，共用 `line-art-legged-slime` 一套模型，差别只在配色与体型参数：

| 原型 | 配色 | 性格 |
| --- | --- | --- |
| `green-legged-slime` | 苔绿 `#a8c489` | 普通个头，中速 |
| `dusk-legged-slime` | 暮紫 `#b0a2c8` | 瘦、腿长、跑得快，血少 |
| `clay-legged-slime` | 陶土 `#d0a184` | 矮胖、走得慢、皮实 |

三种都带 `navigation` 会自己找路追人（见「AI 寻路」），`replicationPolicy` 走 AOI。
**颜色写在原型的 `render` 上，不是刷新时挑的**——换一种颜色就是加一个原型再往变体表里
添一行，刷新那一层一个字都不用改。

无边草原（大世界）与骨骼腿史莱姆两张图上都开了刷新。
