...
生物过去只会沿一条写死的折线走（`patrolPath`）。那条线不认识世界：中间立起一堵墙，
它照旧穿过去。`shared/navigation/` 补的是缺掉的那一半——**看着世界找路**。

结构照着 Minecraft 抄，抄的是分层不是数值：

| SkyLand | Minecraft | 职责 |
| --- | --- | --- |
| `shared/navigation/navNodeEvaluator.mjs` | `WalkNodeEvaluator` | 一格是什么、站多高、边上有没有墙 |
| `shared/navigation/NavRegion.mjs` | `PathNavigationRegion` | 只覆盖搜索窗口的世界快照 |
| `shared/navigation/NavPathfinder.mjs` | `PathFinder` + `BinaryHeap` | A*、节点预算、最近点回退 |
| `shared/navigation/smoothNavPath.mjs` | （无） | 拉绳平滑，把楼梯拉成直线 |
| `shared/actor/components/NavigationComponent.mjs` | `PathNavigation` | 跟着路走、什么时候重寻、卡住了怎么办 |
| `server/actors/NavigationSystem.mjs` | `GoalSelector` 的那一小块 | 挑目标、排搜索的队、写回权威 Transform |

### 导航格就是地形格就是建造格

三套网格共用 2 米这一个尺寸，不是巧合：地形格是地面起伏的最小单位，建造格是地基
占位的最小单位，而寻路要看的正是这两件事。用第三个尺寸，「这一格能不能站人」就要
在三套坐标之间来回换算，每一次换算都是一处可能的分歧。

一格的类型由三层叠出来，后一层盖前一层：

1. **地形**：平地、斜坡、水、岸边（四个正交邻居里有水的陆地格）。
2. **建筑块**：地基把地面抬到自己的顶面——铺在水上就是一条能走的栈桥；篝火之类
   的物件把整格标成危险。
3. **实心碰撞体**：树、石头、别的 Actor。这一层**不写自己的窄相**，直接调玩家移动
   用的那份圆形推出：如果玩家控制器会把你从这里推出去，寻路就不该把你送进来。

墙是唯一一个不占格的东西：它占的是**格边**。所以它单独一个查询（`navEdgeBlocked`）——
墙的碰撞体离两边格心都有一米，只按格分类的寻路会规划出一条穿墙而过的路。斜着穿过
格角要两个中间格都站得住、四条被蹭到的边都没有墙，少判一条就能从墙和树干之间的
缝里插过去。

代价表（malus）跟 Minecraft 同一个约定：`-1` 是「走不了」，正数是「走得了但更贵」。
不会游泳就是 `water: -1`，怕火就是把 `danger` 调高。原型里写 `swim: true` 等于把水从
「走不了」改成一个正数。岸边一格默认比平地贵，陆生 AI 因此宁可往内陆绕一格，而不是
贴着水走。

### 三条闸把成本关在大世界之外

活动范围是 65 公里见方。任何一处按面积分配或遍历的东西都会立刻爆掉，所以这套系统
从设计上就只有三个可能变大的量，每个都有硬上限：

- **窗口**：`NavRegion` 是 `(2r+1)²` 格的定长数组，`r` 上限 48 格（96 米）。窗口外一律
  BLOCKED——那不是近似，就是「这只生物看不了那么远」的定义。工作数组**懒分配**：
  一张没有会寻路 Actor 的地图一个字节都不占，而窗口大小由场景原型决定，不由运行时
  最先寻路的那一只碰巧决定。
- **节点**：一次 A* 最多展开 `maxVisitedNodes` 个节点。死路里的最坏情况因此和一步就到
  的最好情况在同一个量级。
- **队列**：一 tick 最多做 `SEARCHES_PER_TICK` 次搜索，轮转游标保证没有谁一直排在队尾。
  另外，离最近的玩家超过 48 米的生物连目标都不挑——没有人看见的地方，AI 走不走都一样。

分类结果按窗口缓存，并在世界版本号变化时整份作废。版本号来自建造占位表与地形编辑：
**玩家在生物面前放下一堵墙，它手上那条穿墙而过的路当场作废**，而不是等下一个重寻路
周期。

### 找不到路的时候

目标被围死时不是空手而归，而是交出**通往最近点**的那条路。一只站在原地不动的生物
看起来是坏了，一只走到墙根前停下的生物看起来是被墙拦住了。

路径节点带的 `y` 是**决策高度**，只用来判断台阶迈不迈得上、坎敢不敢跳。真正落地的
高度仍然由移动那一侧按精确的 (x, z) 采样地面——拿格心高度当落地高度，走在斜坡上的
生物会一跳一跳。

### 和巡逻的交接

`navigation` 与 `patrolPath` 可以并存：没人可追时巡逻，闻到玩家就追，追完**自己走回
岗位**再把方向盘还回去。追击期间巡逻是被跳过的（它读 `driving` 让位），它的行进进度
因此冻结在生物离开路线的那一刻；直接还回去，巡逻会按那个冻结的进度算出一个位置写进
Transform，生物于是从追到一半的地方瞬移回路线上。所以交接分两步：导航先把它送回那个
点，再把巡逻的进度对齐到它实际站的位置（`PatrolPathComponent.resyncTo`）。那一步位移的
上界是 `NAVIGATION_HANDOVER_RADIUS`，一个小常数，与它追出去多远无关。

### 边界

- 只服务**静态表面**的世界网格。船上的甲板跟着船漂，格坐标是船体本地的，在世界网格里
  没有位置——一只会寻路的生物上不了船。
- 目标只有一个来源：`chase` 写着就追最近的活玩家。AI 追 AI 要的是阵营，而阵营现在还
  不存在，先做出来会是一个没有人用的猜测。
- 会寻路的生物**互相不算障碍**（和 Minecraft 一样，它们靠互相挤开而不是绕开）。把彼此
  当墙的话，一群生物会把自己围死在原地，而且每只看到的世界都不一样，分类缓存也就
  再共用不了。
...
- `shared/navigation/`：AI 寻路的导航格分类、有界搜索窗口、A* 与路径平滑
