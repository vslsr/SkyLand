...
草的频率散布；靠近后按 E 叼住，移动即可拖拽，拉过 `breakLength` 就整株拔断。

拔断的一刻，`mushroomPop` 沿拉拽方向给出线冲量和一份绕水平轴的翻滚冲量
（`spinImpulse`），蘑菇随后作为 Rapier 动态刚体自由飞行、落地、滚动。它的朝向和位置
一样是服务端权威结果：`elasticDetach.rotation` 随快照下发（只在脱落后才占带宽），
客户端在两份快照之间做四元数球面插值，再由 `ActorDropRollSystem` 绕刚体球心摆姿态——
枢轴在球心而不是菌柄根部，蘑菇才是原地翻倒而不是绕着脚跟甩出去。少了这一位，
不管弹多高、滚多远，蘑菇落地永远笔直站着。
