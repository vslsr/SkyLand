...
草簇坐标、朝向和缩放，每簇仍保持三片叶子，因此替换不会改变位置或密度。两条路的叶片
形状都取自 `createGrassBladeGeometry`，材质则共用 `createGrassMaterials.ts` 的同一个
工厂，参数不会在两处各调一遍。

### 草的外观

一根等高、等色、通体描边的直叶子只会长成毛。四件事把它拉回草：

- **根深尖浅的渐变**。场景 JSON 里仍然只有一个 `palette.grass`，`GrassAppearance.ts`
  按 HSL 从它推导根部（更暗更饱和）、叶尖（更亮偏黄）与枯斑三色，再叠上极低频色斑
  噪声混出偏枯的地块。加上根部环境遮蔽和逆光透光，草才有从土里长出来的层次。
- **噪声驱动的高低差**。`GrassNoiseTexture.ts` 生成一张 128² 可平铺的四通道噪声，
  低频通道按世界坐标决定「这一片长得高还是矮」，再乘上逐叶随机打散规整感。贴图是
  确定性的，所以所有客户端看到的高矮团一致；它按 `RepeatWrapping` 取模采样，内存
  与世界面积无关。
- **风过境**。同一张噪声的中频通道沿风向滚动，`smoothstep` 压出前沿，于是风团是
  「扫过去」的一条波而不是整片一起抖；高频通道提供细颤。风向与风力直接取自天气系统
  写入的共享 `windVector`，所以草倒伏的方向和云影漂移的方向必然一致，暴风天自然更猛。
- **弧长补偿**。叶片弯下去会变矮而不是被拉长，踩踏再额外压扁一档。

### 被压过的路径

踩踏不再往一张纹理里逐帧累积，而是记成**数据**：`GrassTrailPath` 是一条定长的
世界坐标点环形缓冲，走满就覆盖最旧的一个点，相距小于最小间距的输入合并进最新点
（原地踏步不会把点数吃光）；每个点按年龄指数回弹，回弹完就丢弃。
`GrassTrailRecorder` 按 `sourceId`（玩家 id、鼠标）分别记录，条数也有上界，
超出时淘汰离焦点最远的一条——两名玩家分头走时并成一条会连出一段谁都没走过的假足迹。

`GrassBendField` 因此变成路径的一个纯函数：每帧清成中性色，再把当前所有路径按线段
实例化盖进那张 32 米窗口的向量场。换来三件以前做不到的事——滑动窗口移动不需要重投影
旧纹理（边缘不再丢压痕，传送也不会拖鬼影）、新流进来的 chunk 立刻带着已有的足迹、
场的内容可以从同步过来的路径重建。成本也从「每帧一次全屏 pass + 每个冲量再来一次」
降到「一次清屏 + 一次实例化绘制」，绘制面积只覆盖足迹本身。

路径定长正是为了能过网络：`encodeGrassTrailPath` 把它量化成 12 字节头 + 每点 8 字节
（坐标是相对路径锚点的厘米增量，半径 2 厘米，强度与年龄各一字节），24 点上界即
204 字节，既不会因为玩家跑得久而变长，也不随世界尺寸增长——存绝对坐标的话 int16
会把世界锁死在 ±327 米以内，远处的足迹将被静默截断；`GrassTrailRecorder.decodePath` 整条替换而不是增量追加，晚到或丢失的
一帧最多让足迹回退一点，不会累出鬼影。**当前运行时还不发这段字节**：远端玩家的
`GrassDisplacementComponent` 在本地按已复制的 transform 重放，各客户端从同一批位置
推出同一条路径，这是最省带宽的一条路。要让离开 AOI 或后加入的客户端也看到既有足迹，
接的就是这套编解码。

玩家踩踏始终可写入当前场景的草地交互目标，鼠标输入则由场景级
`mouse-grass-interaction` Component 独立提供；`open-world.scene.json` 不注册鼠标压草。
...
- **风向量**：水平风向 × 风力也是同一份共享状态。云影漂移、雨雪飘斜和草的倒伏读的是
  同一个值，所以一场暴风里它们不会各吹各的。
...
- `src/grass/`：草地渲染系统、外观参数、噪声贴图、足迹路径与弯曲向量场
